...
import { formatUnits } from "@/lib/onchain/cvault";
import {
...
export function withCommas(ui: string): string {
  ...
  const out =
    frac !== undefined && frac.length > 0 ? `${wholeFmt}.${frac}` : wholeFmt;
  return neg ? `-${out}` : out;
}

export function formatTokenUi(
...
function humanizeVaultState(
  d: Record<string, unknown>,
): Record<string, unknown> {
  const out: Record<string, unknown> = {
    vaultId: d.vaultId,
    totalShares: formatTokenUi(String(d.totalShares ?? "0"), 6),
    totalPendingUsdc: formatUsdUi(
      String(d.totalPendingUsdc ?? "0"),
      USDC_DECIMALS,
    ),
    totalPendingSol: `${formatTokenUi(String(d.totalPendingSol ?? "0"), WSOL_DECIMALS)} SOL`,
    baselineSharePrice: formatUsdUi(
      String(d.baselineSharePrice ?? "0"),
      PRICE_SCALE_DECIMALS,
    ),
    ...
    feeRecipient:
      typeof d.feeRecipient === "string"
        ? shortPubkey(d.feeRecipient)
        : d.feeRecipient,
    address: typeof d.address === "string" ? shortPubkey(d.address) : d.address,
    baseMint:
      typeof d.baseMint === "string" ? shortPubkey(d.baseMint) : d.baseMint,
  };
...
function humanizeUserPosition(
  ...
  if (redeem) {
    ...
    out.pendingUsdc = formatUsdUi(
      String(redeem.pendingUsdc ?? "0"),
      USDC_DECIMALS,
    );
...
function humanizeAssetState(
  d: Record<string, unknown>,
): Record<string, unknown> {
  const priceSource =
    Number(d.priceSourceTag) === 0
      ? "Pyth"
      : Number(d.priceSourceTag) === 1
        ? "DEX"
        : String(d.priceSourceTag);
  const tokenProgram =
  ...
  const out: Record<string, unknown> = {
    ...
    active: d.active,
    pool:
      typeof d.poolAddress === "string"
        ? shortPubkey(d.poolAddress)
        : d.poolAddress,
  };
  if (
    d.pricePoolAddress &&
    typeof d.pricePoolAddress === "string" &&
    d.pricePoolAddress !== "11111111111111111111111111111111"
  ) {
